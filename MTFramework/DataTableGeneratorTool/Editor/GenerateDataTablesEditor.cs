...
public class GenerateDataTablesEditor : EditorWindow
{
    ...
    {
        DataTableProcessor dataTableProcessor = DataTableGenerator.CreateDataTableProcessor(file,Path.GetDirectoryName(file),GetScriptFolderPath(this)+"\\DataTableCodeTemplate.txt");

        if (!DataTableGenerator.CheckRawData(dataTableProcessor, file))
        {
            Debug.LogError(Utility.Text.Format("Check raw data failure. DataTableName='{0}'", file));
        }

        DataTableGenerator.GenerateDataFile(dataTableProcessor, file);
        DataTableGenerator.GenerateCodeFile(dataTableProcessor, file);
    ...
    {
        // 显示现有窗口实例。如果没有，则创建一个。
        EditorWindow.GetWindow(typeof(GenerateDataTablesEditor));
